...
g.test_wrap_with_boolean_error = function()
    local test_capture = Capture:new()
    assert(not test_capture.enabled)
    local ok, err = pcall(function() test_capture:wrap(true, function()
        assert(test_capture.enabled)
    ...
    t.assert_equals(ok, false)
    assert(not test_capture.enabled)
    t.assert_is(err.original, true)
	  t.assert_str_contains(err.traceback, "true")
    t.assert_equals(test_capture:flush(), {stdout = '', stderr = ''})
    t.assert_equals(capture:flush(), {stdout = '', stderr = ''})
end

g.test_wrap_with_errorable__index_error = function()
    local test_capture = Capture:new()
    assert(not test_capture.enabled)
    local ok, err = pcall(function() test_capture:wrap(true, function()
        assert(test_capture.enabled)
        error(setmetatable({"error"}, {__index = error}))
    end) end)
    t.assert_equals(ok, false)
    assert(not test_capture.enabled)
	  t.assert_is(err.original[1], "error")
	  t.assert_str_contains(err.traceback, "error")
    t.assert_equals(test_capture:flush(), {stdout = '', stderr = ''})
    t.assert_equals(capture:flush(), {stdout = '', stderr = ''})
end
